    ...
    <div class="col colusrnf">
        ...
        <div style="overflow-y: scroll; height:400px;" class="scrolldiv">
            <?php
                ...
                $query = mysqli_query($link, "select nom,prenom,mail,login from site.usertemp");
                echo '<p class="bar">------------------------------------------------------------------------------------------------------------</p>';
                while ($row = mysqli_fetch_assoc($query))
                {
                    echo '<p class="textuser">'.$row['mail'].' '.$row['login'].' '.$row['prenom'].' '.$row['nom'].'</p>';
                    echo '<form method="post" action=""><input type="text" name="mail" value='.$row['mail'].' style="display: none;"><input type="submit" value="Bannir" class="btn"></form>';
                    echo '<p class="bar">------------------------------------------------------------------------------------------------------------</p>';
                }
            ?>
            ...
            <?php
                if (isset($_POST['mail'])){
                    ...
                    $ban = mysqli_query($link, "insert into usrban values ('".$_POST['mail']."')");
                    $supp = mysqli_query($link, "delete from usertemp where mail = '".$_POST['mail']."'");
...
<a href="index_admin.html" class="home">
    <h1 class="hhome">
        Accueil
    </h1>
