...
<?php
    if (isset($_POST['txt'])){
        ...
        $login = htmlspecialchars($_SESSIONS['login']);
        ...
        $add = mysqli_query($link, "insert into usertodo values ('".$_SESSION['login']."','".$_POST['txt']."')");

        echo "<script type='text/javascript'>document.location.replace('todo.php');</script>";
    }
?>
...
<div class="index-container">
    <div class="col colusrnf">
        <div class="heading-2">
            <h1>Pense bête de : <?php echo '<span>'.$_SESSION['login'].'</span>'; ?></h1>
        </div>
        <form method="post" action="" class="abc">
            <input type="text" name="txt" placeholder="Nouveau pense bête" class="form-input" required maxlength="299">
            <input type="submit" value="Ajouter" class="btn">
        </form>
        <div style="overflow-y: scroll; height:400px;" class="scrolldiv">
            <?php
                $link = mysqli_connect('localhost', 'root', 'lannion') or die ('Error connecting to mysql: ' . mysqli_error($link).'\r\n');
                ...
                $query = mysqli_query($link, "select txt from site.usertodo where login = '".$login."'");
                
                while ($row = mysqli_fetch_assoc($query))
                {
                    echo '<p class="texttodo">'.$row['txt'].'</p>';
                }
            ?>
        </div>
    </div>
...
<a href="index_all.php" class="home">
    <h1 class="hhome">
        Accueil
    </h1>
